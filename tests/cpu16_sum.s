...
# register, so the two together build any 8 bit constant.  "la <dst> <label>"
# is the assembler pseudo instruction that puts a label's address into a
# register, which is how a branch target is named now.
